...
import { getWeeklyResults } from '../services/leaderboardService';
...
const WinnersPage: React.FC = () => {
    ...
    useEffect(() => {
        const results = getWeeklyResults().sort((a, b) => b.week - a.week); // Show most recent first
    ...
    const handleOpenModal = (winner: WeeklyWinnerInfo, rank: number) => {
        ...
        setSelectedWinner({
            name: winner.name,
            rank,
            title: getPlaceholderTitle(currentResult.week, rank)
        });
        setIsModalOpen(true);
    };
                 ...
                 {currentResult ? (
                    <div className="w-full max-w-5xl animate-fade-in-up" style={{animationDelay: '0.2s'}}>
                        <div className="flex justify-between items-center mb-8 px-4">
                            ...
                            <h2 className="text-2xl sm:text-3xl font-bold text-amber-800 dark:text-yellow-200 text-center">Week {currentResult.week}</h2>
